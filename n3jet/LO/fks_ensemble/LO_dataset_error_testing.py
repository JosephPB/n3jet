...
if model_dir != '':
    model_dir = model_dir + '/{}_{}_{}_{}_{}/dataset_size/'.format(order,n_gluon+2,delta_cut,delta_near,points)
...
if os.path.exists(data_dir + 'PS_cut_{}_{}_{}.npy'.format(n_gluon+2,delta_cut,test_points)) == False and os.path.exists(data_dir + 'PS_near_{}_{}_{}.npy'.format(n_gluon+2,delta_near,test_points)) == False: 
    print ('No directory: {}'.format(data_dir + 'PS_cut_{}_{}_{}.npy'.format(n_gluon+2,delta_cut,test_points)))
    print ('############### Creating both cut and near phasespace test_points ###############')
    cut_momenta, near_momenta = rpb.generate(n_gluon+2,test_points,1000., delta_cut, delta_near)
...
if os.path.exists(data_dir + 'NJ_{}_cut_{}_{}_{}.npy'.format(order, n_gluon+2, delta_cut, test_points)) == False\
    and os.path.exists(data_dir + 'NJ_{}_near_{}_{}_{}.npy'.format(order, n_gluon+2, delta_near, test_points)) == False:
    ...
    np.save(data_dir + 'NJ_{}_cut_{}_{}_{}'.format(order, n_gluon+2, delta_cut, test_points),np.array(NJ_cut_treevals))
    np.save(data_dir + 'NJ_{}_near_{}_{}_{}'.format(order, n_gluon+2, delta_near, test_points),np.array(NJ_near_treevals))
...
elif os.path.exists(data_dir + 'NJ_{}_cut_{}_{}_{}.npy'.format(order, n_gluon+2, delta_cut, test_points)) == False:
    print ('############### Calculating cut njet test_points ###############')
    NJ_cut_treevals, NJ_near_treevals = generate_LO_piecewise_njet(test_cut_momenta, test_near_momenta, test_data)
    np.save(data_dir + 'NJ_{}_cut_{}_{}_{}'.format(order, n_gluon+2, delta_cut, test_points),np.array(NJ_cut_treevals))
    
elif os.path.exists(data_dir + 'NJ_{}_near_{}_{}_{}.npy'.format(order, n_gluon+2, delta_near, test_points)) == False:
...
NJ_cut_test_treevals = np.load(data_dir + 'NJ_{}_cut_{}_{}_{}.npy'.format(order, n_gluon+2, delta_cut, test_points), allow_pickle=True)
NJ_near_test_treevals = np.load(data_dir + 'NJ_{}_near_{}_{}_{}.npy'.format(order, n_gluon+2, delta_near, test_points), allow_pickle=True)
...
pairs, NJ_near_test_treevals_split = multiprocess_weighting(test_near_momenta,n_gluon,NJ_near_test_treevals,cores)
    
    
NN = Model((n_gluon+2-1)*4,test_near_momenta, NJ_near_test_treevals_split[0])
...
test_momenta_array = np.concatenate((test_cut_momenta,test_near_momenta))
NJ_test = np.concatenate((NJ_cut_test_treevals,NJ_near_test_treevals))
y_preds = []
for i in range(training_reruns):
...
plt.savefig(model_dir + '/cs_convergence_{}.png'.format(test_points), dpi = 250,bbox_inches='tight')
plt.close()
...
plt.savefig(model_dir + '/cs_mean_convergence_{}.png'.format(test_points), dpi = 250,bbox_inches='tight')
plt.close()
...
plt.legend()
if order == 'NLO':
    plt.title('Average {} '.format(order)+r'differential virtual correction against $p_T$ for $e^+e^-\rightarrow\,q\bar{q}$'+'{} w/ FKS'.format(n_gluon*'g'))
else:
    plt.title('Average {} '.format(order)+r'differential cross section against $p_T$ for $e^+e^-\rightarrow\,q\bar{q}$'+'{} w/ FKS'.format(n_gluon*'g'))
plt.savefig(model_dir + '/cs_mean_convergence_stderr_{}.png'.format(test_points), dpi = 250,bbox_inches='tight')
plt.close()
