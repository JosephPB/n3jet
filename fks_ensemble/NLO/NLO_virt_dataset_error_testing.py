...
if model_dir != '' and delta_renear == 0. and delta_recut == 0.:
    model_dir = model_dir + '/{}_virt_{}_{}_{}_{}/dataset_size/'.format(order,n_gluon+2,delta_cut,delta_near,points)
elif model_dir != '' and delta_renear != 0. and delta_recut != 0.:
    model_dir = model_dir + '/{}_virt_{}_{}_{}_{}/dataset_size/'.format(order,n_gluon+2,delta_recut,delta_renear,points)
...
NLO = np.load(data_dir + 'NJ_NLO_{}_{}_{}.npy'.format(n_gluon+2, delta, test_points), allow_pickle=True)
...
plt.legend()
if order == 'NLO':
    plt.title('{} '.format(order)+r'differential virtual correction against $p_T$ for $e^+e^-\rightarrow\,q\bar{q}$'+'{} w/ FKS'.format(n_gluon*'g'))
else:
