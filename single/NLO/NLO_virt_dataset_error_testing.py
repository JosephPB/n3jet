...
if model_dir != '':
    model_dir = model_dir + '/{}_virt_{}_{}_{}/dataset_size/'.format(order,n_gluon+2,redelta,points)
...
plt.legend()
if order == 'NLO':
    plt.title('{} '.format(order)+r'virtual correction for $e^+e^-\rightarrow\,q\bar{q}$'+'{} w/ FKS'.format(n_gluon*'g'))
else:
...
plt.legend()
if order == 'NLO':
    plt.title('Average {} '.format(order)+r'virtual correction for $e^+e^-\rightarrow\,q\bar{q}$'+'{} w/ FKS'.format(n_gluon*'g'))
else:
